...
def get_support_pivots(candles: list, lookback_left, lookback_right):
    support_pivots = []
    for i, candle in enumerate(candles):
        candle_index = i
        start_index = candle_index - lookback_left
        end_index = candle_index + lookback_right + 1
...
def get_lows(candles, lookback_left, lookback_right):
    lows = get_support_pivots(candles, lookback_left, lookback_right)
    for pivot in lows:
        for i in lows:
...
def get_resistance_pivots(candles: list, lookback_left: int, lookback_right: int):
    resistance_pivots = []
    for i, candle in enumerate(candles):
        candle_index = i
        start_index = candle_index - lookback_left
...
def get_highs(candles, lookback_left, lookback_right):
    highs = get_resistance_pivots(candles, lookback_left, lookback_right)
    for pivot in highs:
        for i in highs:
